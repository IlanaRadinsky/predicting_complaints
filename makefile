...
features_with_gloominess.csv: features.csv construct_gloominess_index.awk
	echo "date,avg_temp,daylight_hours,bad_weather,hours_overcast,gloomy_today,gloomy_past_two_days,gloomy_past_three_days" > features_with_gloominess.csv
	./construct_gloominess_index.awk >> features_with_gloominess.csv
...
dow.csv: perc_noise_by_date.csv get_day_of_week.awk
	echo "date,day_of_week" > dow.csv
	cat perc_noise_by_date.csv | ./get_day_of_week.awk >> dow.csv

final.csv: features_with_gloominess.csv complaints_by_date.csv
	join --header -1 1 -2 1 -t',' features_with_gloominess.csv complaints_by_date.csv > final.csv

final_noise.csv: features_with_gloominess.csv perc_noise_by_date.csv
	join --header -1 1 -2 1 -t',' features_with_gloominess.csv perc_noise_by_date.csv > final_noise.csv
